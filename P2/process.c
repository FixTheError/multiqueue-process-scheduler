...
// to happen).
static Queue	runQueues[NUM_OF_RUNQUEUE];
...
ProcessSchedule ()
{
  ...
  dbprintf ('p', "Now entering ProcessSchedule (cur=0x%x, %d ready)\n",
            currentPCB, QueueLength (&runQueues[currentPCB->priority]));
  // The OS exits if there's no runnable process.  This is a feature, not a
  // bug.  An easy solution to allowing no runnable "user" processes is to
  // have an "idle" process that's simply an infinite loop.
  for (i = 0; i < NUM_OF_RUNQUEUE; i++) {
	  if (!QueueEmpty(&runQueues[i])) {
		  pcb = (PCB*)((QueueFirst(&runQueues[i]))->object);
		  qlen = QueueLength(&runQueues[i]);
		  do {
			  if (pcb != currentPCB) {
				  pcb->p_info += 1;
			  }
			  ...
			  if (pcb->p_info >= 300) {
				  QueueRemove(&pcb->l);
				  pcb->priority = 15;
				  pcb->quantum = 2 * (pcb->baseQuantum);
				  pcb->p_info = 0;
				  pcb->readytoolong = 1;
				  QueueInsertLast(&runQueues[pcb->priority], &pcb->l);
				  pcb = (PCB*)((QueueFirst(&runQueues[i]))->object);
			  }
			  else {
				  QueueRemove(&pcb->l);
				  QueueInsertLast(&runQueues[i], &pcb->l);
				  pcb = (PCB*)((QueueFirst(&runQueues[i]))->object);
			  }
			  pcount++;
		  } while (pcount < qlen);
  ...
  i = 31;
  while (QueueEmpty(&runQueues[i]) && i >= 0) {
	  if (QueueEmpty(&runQueues[i])) {
		  emptycount++;
	  }
	  if (emptycount == NUM_OF_RUNQUEUE) {
  ...
  if (pcb->quantum <= 0) {
	  if (pcb->readytoolong = 0 && pcb->priority != pcb->basePriority) {
		  QueueRemove(&pcb->l);
		  pcb->priority--;
		  pcb->quantum = pcb->baseQuantum;
		  QueueInsertLast(&runQueues[pcb->priority], &pcb->l);
	  }
	  else if (pcb->readytoolong = 1) {
		  pcb->readytoolong = 0;
		  QueueRemove(&pcb->l);
  ...
  pcount = 0;
  pcb = (PCB *)((QueueFirst (&runQueues[i]))->object);
  while (!(pcb->flags & PROCESS_STATUS_RUNNABLE)) {
	  
	  if (pcount == QueueLength(&runQueues[i])) {
		  i--;
		  if (i < 0) {
			  i = 31;
...
ProcessWakeup (PCB *wakeup)
{
  ...
  QueueInsertLast (&runQueues[wakeup->priority], &wakeup->l);
...
ProcessFork (VoidFunc func, uint32 param, int p_nice, int p_info,char *name, int isUser)
{
  ...
  //---------------------------------------
  // Lab3: initialize pcb members for your scheduling algorithm here
  //--------------------------------------
  pcb->p_nice = p_nice;
  pcb->p_info = p_info;
  pcb->basePriority = 15 - p_nice;
  pcb->priority = pcb->basePriority;
  pcb->baseQuantum = 6;
  pcb->quantum = pcb->baseQuantum;
  //----------------------------------------------------------------------
  ...
  intrs = DisableIntrs ();
  QueueInsertLast (&runQueues[pcb->priority], l);
  RestoreIntrs (intrs);
